...
function FAQ() {
  ...
  const faqs = [
    ...
    { q: "What is SmileCRM?", a: "SmileCRM is our purpose-built CRM for independent dental clinics. It automates appointment reminders, patient recall, Google review requests, and WhatsApp follow-ups. Pricing starts at ₹2,999/month." },
    { q: "What is the AI BDR product?", a: "AI BDR is a fully automated business development system that researches leads, writes personalised outreach emails, follows up automatically, and hands you warm replies. It runs about 200 personalised sequences a month with a 48-hour setup, starting at ₹15,000/month." },
    { q: "Do you offer an AI voice receptionist?", a: "Yes — our AI Voice Receptionist answers your business phone 24/7, books appointments, handles common questions, and escalates to a human when needed. It's currently on waitlist, starting at ₹2,999/month." },
    { q: "What's the AI Business Ecosystem?", a: "It's our end-to-end automation package for any SMB: CRM setup, workflow automation with n8n/Make.com, custom AI agents, lead capture, and a monthly strategy review — all connected and running together. Pricing is custom based on scope." },
    { q: "What industries do you work with?", a: "We work with healthcare and dental clinics, gyms and wellness studios, home services and trades (HVAC, plumbing, electrical), e-commerce and D2C brands, agencies and consultancies, and any growing service business that's outgrown its current systems." },
    { q: "How does the free diagnostic work?", a: "It takes about 3 minutes, needs no credit card, and scores your business across 9 dimensions with instant results shown as a radar chart — so you can see exactly where you're leaking revenue before talking to us." },
    { q: "What happens on the discovery call?", a: "It's a free 30-minute call (booked via our calendar) where we look at your specific processes and identify at least one actionable quick win. If we can't, you get our DX Roadmap Template free." },
    { q: "Do you take on every client who applies?", a: "No — we deliberately cap it at a maximum of 5 new clients per month so every engagement gets real attention rather than being spread thin." },
    { q: "How do WhatsApp automations work?", a: "We're a registered WhatsApp Tech Provider, so reminders, recall messages, and follow-ups (for example through SmileCRM) are sent via official WhatsApp Business channels rather than unofficial workarounds." },
    { q: "Can you automate patient recall and no-show reduction for a dental clinic?", a: "Yes — this is exactly what SmileCRM and our dental automation service handle: automated appointment reminders, automated patient recall, and WhatsApp & SMS reminders to cut no-shows." },
    { q: "Do you help with online reviews?", a: "Yes — Google review automation is a core part of our dental and clinic offerings, prompting satisfied patients/customers to leave reviews after their visit." },
    { q: "What if I run a gym or fitness studio, not a clinic?", a: "We have a dedicated fitness & wellness solution focused on member retention, class scheduling, and revenue optimization for gyms and studios." },
    { q: "What if I run an e-commerce or D2C brand?", a: "We handle order fulfillment, customer service, and inventory management automation for e-commerce and D2C brands." },
    { q: "Do you build custom AI agents, or only use off-the-shelf tools?", a: "Both — the AI Business Ecosystem package includes custom AI agents built for your workflows on top of n8n/Make.com automations, not just pre-made templates." },
    { q: "How do I get started?", a: "Start with the free 3-minute diagnostic, or book a 30-minute discovery call directly — whichever you're more comfortable with. There's no cost and no obligation either way." },
  ];
  const faqJsonLd = {
...
function Nav() {
  ...
  const links = [["How It Works", "how"], ["Services", "services"], ["Clinic Score", "clinic-score"], ["Store", "store"], ["Blog", "blog"], ["About", "about"], ["FAQ", "faq"]];
  const NavBadge = () => <span style={{ marginLeft: 6, background: "#22c55e", color: "#fff", fontSize: 9, fontWeight: 800, padding: "1px 6px", borderRadius: 20, letterSpacing: "0.04em", verticalAlign: "middle" }}>NEW</span>;
  const isPageLink = (id) => id === "blog" || id === "store" || id === "about" || id === "clinic-score";
  return (
    <nav style={{ position: "fixed", top: 44, left: 0, right: 0, zIndex: 150, background: sc ? "rgba(255,255,255,0.97)" : "transparent", backdropFilter: sc ? "blur(12px)" : "none", borderBottom: sc ? "1px solid #E5E7EB" : "none", transition: "all 0.3s" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "13px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div className="dln" style={{ display: "flex", alignItems: "center", gap: 24 }}>
          {links.map(([l, id]) => { const lc = sc ? "#4B5563" : "#E2E8F0"; return isPageLink(id) ? <a key={id} href={`/${id}`} style={{ display: "flex", alignItems: "center", background: "none", border: "none", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 500, color: lc, transition: "color 0.15s", textDecoration: "none" }} onMouseEnter={e => e.currentTarget.style.color = "#2563EB"} onMouseLeave={e => e.currentTarget.style.color = lc}>{l}{id === "clinic-score" && <NavBadge />}</a> : <button key={id} onClick={() => scrollTo(id)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontSize: 14, fontWeight: 500, color: lc, transition: "color 0.15s" }} onMouseEnter={e => e.target.style.color = "#2563EB"} onMouseLeave={e => e.target.style.color = lc}>{l}</button>; })}
      ...
      {m && <div style={{ background: "#fff", padding: "8px 24px 20px", borderTop: "1px solid #E5E7EB" }}>
        {links.map(([l, id]) => isPageLink(id) ? <a key={id} href={`/${id}`} style={{ display: "flex", alignItems: "center", width: "100%", textAlign: "left", padding: "16px 0", minHeight: 48, fontFamily: "'DM Sans',sans-serif", fontSize: 16, fontWeight: 500, color: "#374151", textDecoration: "none", borderBottom: "1px solid #F3F4F6" }}>{l}{id === "clinic-score" && <NavBadge />}</a> : <button key={id} onClick={() => { scrollTo(id); setM(false); }} style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", padding: "16px 0", minHeight: 48, fontFamily: "'DM Sans',sans-serif", fontSize: 16, fontWeight: 500, color: "#374151", cursor: "pointer", borderBottom: "1px solid #F3F4F6" }}>{l}</button>)}
